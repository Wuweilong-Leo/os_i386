...
#define BLA_BACK_WHI_WORD 0x07
#define SET_VIDEO_MEM(offset, val) *(video_base_addr + (uint32_t)(offset)) = val
...
static uint16_t get_cursor(){
    ...
    uint8_t cur_pos_low_index = CUR_POS_LOW_INDEX;
    asm volatile ("outb %%al, %%dx"::"d"(crt_addr_reg), "a"(cur_pos_high_index));
    asm volatile ("inb %%dx, %%al": "=a"(cur_pos_high): "d"(crt_data_reg));
    asm volatile ("outb %%al, %%dx"::"d"(crt_addr_reg), "a"(cur_pos_low_index));
    asm volatile ("inb %%dx, %%al": "=a"(cur_pos_low): "d"(crt_data_reg));
    return ((uint16_t)cur_pos_high << 8)& 0xff00 + cur_pos_low;
}

static void roll_screen(void* dst, void* src, uint32_t size){
...
static void clean_last_line(){
    ...
    for(uint32_t i = 0; i < 80; i++){
        SET_VIDEO_MEM(offset, ' ');
        SET_VIDEO_MEM(++offset, BLA_BACK_WHI_WORD);
        offset++;
    }
}

#define CHECK_OUT_OF_SCREEN(cur_pos)\
    do{\
        if(cur_pos >= 2000){\
            roll_screen(roll_video_dst_addr, roll_video_src_addr, 960 * 4);\
            clean_last_line();\
            next_cur_pos = 1920;\
        }\
    }while(0);
...
void put_char(uint8_t c){
    ...
    if(c == '\r' || c == '\n'){
        ...
        SET_VIDEO_MEM(offset, ' ');
        SET_VIDEO_MEM(offset + 1, BLA_BACK_WHI_WORD);
        next_cur_pos = cur_pos;
    }else{
        uint32_t offset = cur_pos * 2;
        video_base_addr[offset] = c;
        video_base_addr[offset + 1] = BLA_BACK_WHI_WORD;
        next_cur_pos = cur_pos + 1;
        CHECK_OUT_OF_SCREEN(cur_pos);
    }
